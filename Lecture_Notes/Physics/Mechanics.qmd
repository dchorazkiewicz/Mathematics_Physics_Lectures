...
$$
\mathbf{a}(t) = \frac{d\mathbf{v}(t)}{dt} = -R \omega^2 \cos(\omega t) \hat{i} - R \omega^2 \sin(\omega t) \hat{j} = -\omega^2 \mathbf{r}(t)
$$
...
This result also has an important physical interpretation. Motion along a circle is possible only if the acceleration is directed toward the center of the circle. This is a kinematic fact: even if the speed is constant, the direction of the velocity is continuously changing, so the velocity vector must have a nonzero derivative pointing inward.

By Newton's second law, an inward acceleration requires an inward net force:

$$
\mathbf{F}_{\text{centripetal}} = m \mathbf{a}
$$

and therefore

$$
|\mathbf{F}_{\text{centripetal}}| = m \frac{v^2}{R}.
$$

This force is called the **centripetal force**. It is not a new fundamental force. It is simply the name for whatever physical force provides the inward acceleration needed to keep the object on a circular path. In later sections we will return to this point and see that, in orbital motion, gravity can play exactly the role of the centripetal force.
...
In three-dimensional motion, the work ([app](https://dchorazkiewicz.github.io/Physics_Problems_Repo/anim/work_integral.html)) done by a force $\mathbf{F}$ acting on an object as it moves along a displacement $d\mathbf{r}$ is given by the **dot product**:
...
To make the next step clear, let us look at one component, for example the $x$-direction:

$$
a_x dx
$$

Since acceleration is the derivative of velocity and velocity is the derivative of position, we have:
...
Substituting this into $a_x dx$ gives:

$$
a_x dx = \frac{dv_x}{dt} \, dx
= \frac{dv_x}{dt} \, (v_x dt)
= v_x \, dv_x
$$

Exactly the same argument works for the $y$ and $z$ components:
...
### Kinetic Energy
To determine the total work done from an initial velocity $\mathbf{v}_1 = (v_{1x}, v_{1y}, v_{1z})$ to a final velocity $\mathbf{v}_2 = (v_{2x}, v_{2y}, v_{2z})$, we integrate:
...
To see formally why this implies **path independence**, consider an infinitesimal displacement
$d\mathbf{r} = (dx,dy,dz)$. The differential of the potential energy is
...
Using $\mathbf{F} = -\nabla U$, the infinitesimal work becomes
...
Now integrate from point $A$ to point $B$ along any path $\mathcal{C}$:
...
and this depends only on the values of the potential at the endpoints, not on the path $\mathcal{C}$ itself.

In particular, for any closed path:

$$
\oint \mathbf{F}\cdot d\mathbf{r} = 0
$$

because the initial and final points are the same, so the change in potential energy is zero.

Combining this with the work-kinetic energy theorem,

$$
W = \Delta KE
$$

we get for conservative forces
...
For the basic cases discussed above, the total mechanical energy takes the following forms:

| Case | Kinetic energy $KE$ | Potential energy $U$ | Total energy $E = KE + U$ |
|---|---|---|---|
| Uniform gravity near Earth | $\frac{1}{2}mv^2$ | $mgy$ | $E = \frac{1}{2}mv^2 + mgy$ |
| Harmonic oscillator | $\frac{1}{2}mv^2$ | $\frac{1}{2}kx^2$ | $E = \frac{1}{2}mv^2 + \frac{1}{2}kx^2$ |
| Newtonian gravity | $\frac{1}{2}mv^2$ | $-\frac{GmM}{r}$ | $E = \frac{1}{2}mv^2 - \frac{GmM}{r}$ |
...
We now return to gravity with the language of work, kinetic energy, potential energy, and energy conservation already available. This will allow us not only to describe the gravitational force itself, but also to understand orbital motion and escape velocity in a more natural and physically motivated way.
...
Let us now connect this discussion with the previous section on circular motion. We have already seen that any motion along a circle requires an inward acceleration of magnitude

$$
\frac{v^2}{r},
$$

so by Newton's second law it also requires an inward net force of magnitude

$$
m \frac{v^2}{r}.
$$

In orbital motion this inward force is not supplied by a string, a wall, or mechanical contact. Instead, it is provided by gravity. In other words, for a body moving on a circular orbit, gravity plays the role of the centripetal force.
...
Escape velocity is the minimum initial velocity required for an object to escape the gravitational pull of a planet or celestial body without further propulsion. More precisely, it is the threshold velocity for which the object can move arbitrarily far away and never return, assuming that we neglect air resistance and other dissipative effects.

This does not mean that gravity suddenly disappears beyond some distance. Gravity acts at all distances. The point is instead that the object has enough total mechanical energy to avoid remaining gravitationally bound to the planet.

We now derive the formula using energy conservation. Suppose an object of mass $m$ is launched from the surface of a body of mass $M$ and radius $R$ with initial speed $v$.

Its initial mechanical energy is

$$
E_i = \frac{1}{2} m v^2 - \frac{G M m}{R}.
$$

For the limiting case of escape, the object reaches arbitrarily large distance with final speed equal to zero. Since the gravitational potential energy tends to zero at infinity, the final mechanical energy is
...
This value has a clear interpretation:

- if $v < v_\text{escape}$, the object remains gravitationally bound and will eventually fall back,
- if $v = v_\text{escape}$, the object escapes and reaches infinity with zero final speed,
- if $v > v_\text{escape}$, the object escapes with positive residual speed even very far from the planet.

It is also useful to compare escape velocity with circular orbital velocity:

$$
v_\text{orbital} = \sqrt{G \frac{M}{R}}.
$$
...
So the escape velocity is larger than the circular orbital velocity by the factor $\sqrt{2}$.

For reference, the escape velocities from the surfaces of selected bodies in the Solar System are approximately:
