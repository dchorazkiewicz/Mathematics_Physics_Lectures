...
\paragraph{Example: Proof of the limit of $f(x) = x^2$}
We will prove that $\lim_{x \to 3} x^2 = 9$.

\textbf{Proof:}
According to the $\varepsilon$-$\delta$ definition of a limit, we need to show that for any given $\varepsilon > 0$, there exists a $\delta > 0$ such that for all $x$, if $0 < |x - 3| < \delta$, then $|x^2 - 9| < \varepsilon$.

Let $\varepsilon > 0$ be given. We want to find a $\delta > 0$. Let's analyze the expression $|x^2 - 9|$:
\[ |x^2 - 9| = |(x-3)(x+3)| = |x-3||x+3| \]
We need to bound the term $|x+3|$. We are interested in values of $x$ close to 3, so we can restrict $\delta$ to be less than or equal to 1.
If we assume $\delta \le 1$, then from $|x - 3| < \delta$, we have $|x - 3| < 1$.
This implies $-1 < x - 3 < 1$, which leads to $2 < x < 4$.
For $x$ in this interval, we have $5 < x+3 < 7$, so $|x+3| < 7$.

Now we can write:
\[ |x^2 - 9| = |x-3||x+3| < 7|x-3| \]
To make this less than $\varepsilon$, we require $7|x-3| < \varepsilon$, which means $|x-3| < \varepsilon/7$.

So, we need to satisfy two conditions for $\delta$: $\delta \le 1$ and $\delta \le \varepsilon/7$. We can choose $\delta$ to be the smaller of these two values:
\[ \delta = \min\left(1, \frac{\varepsilon}{7}\right) \]
Now, let's verify this choice. For any given $\varepsilon > 0$, let $\delta = \min(1, \varepsilon/7)$.
If $0 < |x-3| < \delta$, then we have:
1. $|x-3| < 1$, which we have shown implies $|x+3| < 7$.
2. $|x-3| < \varepsilon/7$.

Combining these, we get:
\[ |x^2 - 9| = |x-3||x+3| < \left(\frac{\varepsilon}{7}\right) \cdot 7 = \varepsilon \]
Thus, we have shown that for any $\varepsilon > 0$, we can find a $\delta > 0$ such that if $0 < |x-3| < \delta$, then $|x^2 - 9| < \varepsilon$. This completes the proof.
\hfill $\blacksquare$